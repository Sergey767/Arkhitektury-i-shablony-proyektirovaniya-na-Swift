...
class ParseData: Operation {
    ...
    func loadGroups(token: String, delegate: VkApiGroupsDelegate) {
        ...
        ParseData.session.request(baseUrl + path, method: .get, parameters: params).responseJSON(
            queue: DispatchQueue.global(qos: .userInteractive)) { response in
                let groups = VkResponseParser.instance.parseGroups(token: token, result: response.result)
